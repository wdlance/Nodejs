...
router.post('/', function(req, res, next) {
	...
	if (originalUrl.indexOf("getAll") != -1) {
		...
		if (files.length != 0) {
			var url = 'public\\info\\body\\原始所有图片文件.txt';
		...
		var src = req.body.src;
		var url = 'public\\info\\body\\_negative.txt'
		var person_url = "public\\info\\body\\_person.txt"
		fs.readFile(person_url, { encoding: 'utf8' }, function(err, data) {
			if (err) {
				console.log(err)
			}
			if (data.indexOf(src) != -1) {
				...
				writeFile(res, person_url, JSON.stringify(newArr))
			}
		})
    ...
    form.parse(req)
        ...
        .on('end', function() {
        	var path = 'public\\info\\body\\_masked';
        	var url = 'public\\info\\body\\_mask.txt';
        		...
        		if (exists) {
        			...
        			if (JSON.parse(masks).length > 0) {
        				...
        				fs.writeFile(url, data, function(err) {
        					...
        					saveMaskInfo(path,file,res,imgPath,masks)
        				})
        			}else{
        				res.send({
        					ret:0,
    	...
    	var url = 'public\\info\\body\\_person.txt';
    	var info = {
    		src: src,
    		rects: rects,
    	};
    	var negative_url = "public\\info\\body\\_negative.txt"
    	...
    	var src = req.body.src;
    	var url = 'public\\info\\body\\_person.txt';
    	var mask_url = "public\\info\\body\\_mask.txt";
...
function saveMaskInfo(path,file,res,imgPath,masks,src){
	...
	if (JSON.parse(masks).length > 0) {
		...
		var del_path = 'public\\info\\body\\_masked\\' + newArr
